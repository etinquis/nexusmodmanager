	...
	public class ApplicationInitializer : ThreadedBackgroundTask
	{
		...
		protected bool UacCheckEnvironment(IGameMode p_gmdGameMode, out ViewMessage p_vwmErrorMessage)
		{
			...
			foreach (KeyValuePair<string, string> kvpUacCheckPath in dicPaths)
			{
				...
				if (!Directory.Exists(kvpUacCheckPath.Key))
				{
					Trace.TraceError("Unable to access: " + kvpUacCheckPath.Key);
					string strMessage = "Unable to access:" + Environment.NewLine + kvpUacCheckPath.Key;
					string strDetails = String.Format("This error usually happens when you set one of the {0} folders on a hard drive that is no longer in your system:" +
					...
					p_vwmErrorMessage = new ViewMessage(strMessage, strDetails, "Warning", MessageBoxIcon.Warning);
